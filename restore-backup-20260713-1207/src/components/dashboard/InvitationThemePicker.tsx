...
import { THEME_INFO, type ThemeName } from '@/contexts/ThemeContext';
...
export default function InvitationThemePicker() {
  const { theme: currentThemeObj, setTheme } = useTheme();
  const theme = currentThemeObj.id as ThemeName;
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'border-accent bg-accent/10 shadow-luxury scale-[1.02]'
                      : 'border-border/40 hover:border-accent/50 hover:bg-muted/10 hover:shadow-surface'
                  )}
                  style={{
                    background: isActive
                ...
                >
                  {isActive && (
                    <span className="absolute right-3 top-3 rounded-full bg-accent text-accent-foreground px-2 py-1 text-[10px] uppercase tracking-[0.18em]">
                      Active
                    </span>
                  )}
