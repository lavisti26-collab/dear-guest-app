...
function Inner() {
  ...
  const [superAdminUserId, setSuperAdminUserId] = useState<string | undefined>(undefined);
  ...
  const handleDeleteCouple = async (userId: string, displayName: string) => {
    if (!confirm(`Delete ALL data for "${displayName}"?\n\nThis permanently removes their profile, guests, wishes, photos, settings and program.\nThis cannot be undone.`)) return;
    ...
    try {
      // 1️⃣ Try the SECURITY DEFINER RPC first (bypasses RLS cleanly)
      ...
      if (rpcError) {
        // RPC itself failed (network / function not found / permission)
        console.error('RPC error:', rpcError);
        // 2️⃣ Fallback: direct sequential deletes (RLS allows super admins)
        const couple = couples.find(c => c.user_id === userId);
        const email = couple?.email;
        if (email) {
          await supabase.from('access_requests').delete().eq('email', email);
        }
        await supabase.from('program_schedule').delete().eq('user_id', userId);
        ...
        if (profileErr) {
          ...
          toast.error(`Delete failed: ${profileErr.message}`);
          return;
        }
      } else if (rpcData?.error) {
        // RPC ran but returned a logical error (e.g. not super admin)
        console.error('RPC returned error:', rpcData.error);
        toast.error(`Delete failed: ${rpcData.error}`);
        return;
      }
      ...
      toast.error(`Delete error: ${err?.message || 'Unknown error — check console'}`);
    }
  };
  ...
  return (
    <ThemeProvider initialTheme={resolveLegacyTheme(getThemeForRole('super_admin'))}>
    <VisualStyleProvider ownerUserId={superAdminUserId}>
    <CinematicTransition>
      <div className="min-h-screen bg-background p-6">
        ...
        <header className="max-w-6xl mx-auto mb-6 flex items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link to="/admin?tab=theme" className="text-sm rounded-full border border-border bg-primary/10 px-4 py-2 text-primary hover:bg-primary/20 transition-colors">
              Design studio
            </Link>
