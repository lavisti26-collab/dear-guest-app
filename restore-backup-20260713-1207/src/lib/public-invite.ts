...
export async function fetchPublicProfileBySlug(
  ...
  // If there was no query error but both returned empty, it's truly not found.
  // Otherwise it's a database or network connection error.
  const isQueryError = rpcError || viewError;
  return {
    profile: null,
    error: isQueryError 
      ? (rpcError?.message || viewError?.message || 'Connection error') 
