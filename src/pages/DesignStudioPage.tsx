...
const DesignStudioPage = () => {
  const [themeId, setThemeId] = useState<V2ThemeId>('royal-khmer');
  ...
  const theme = useMemo(
    () => V2_THEMES.find((item) => item.id === themeId) ?? V2_THEMES[0],
    [themeId]
  );
                      ...
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex gap-1">
                          {option.swatches.map((swatch) => (
                            <span key={swatch} className="h-5 w-5 rounded-full border border-white/10" style={{ backgroundColor: swatch }} />
                          ))}
                        </div>
                      </div>
                    </button>
                ...
                <div className="rounded-[2rem] border border-white/10 bg-black/40 p-6" style={previewStyle}>
                  <div className="flex items-center justify-between gap-4 pb-4">
                    <div>
                      ...
                      <h3 className="mt-2 text-2xl font-semibold" style={{ fontFamily: typography.heading === 'elegant-script' ? 'serif' : 'system-ui' }}>
                        {theme.label}
                      </h3>
                    </div>
                  ...
                  <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6" style={{ backgroundColor: theme.colors.card }}>
                    <div className="space-y-6">
                      <div>
                        <p className="text-sm uppercase tracking-[0.35em] text-slate-400">Hero section</p>
                        <h1 className="mt-4 text-4xl font-semibold leading-tight" style={{ fontFamily: typography.heading === 'elegant-script' ? 'serif' : 'system-ui', color: theme.colors.primary }}>
                          Your unforgettable celebration.
                        </h1>
                        <p className="mt-4 max-w-2xl text-base text-slate-300" style={{ fontFamily: typography.body === 'modern-sans' ? 'system-ui' : 'Georgia, serif' }}>
                      ...
                      <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-5">
                        ...
                        <blockquote className="mt-4 text-base italic leading-8 text-slate-200" style={{ fontFamily: typography.quote === 'editorial' ? 'Georgia, serif' : 'system-ui' }}>
                ...
                <div className="rounded-3xl border border-white/10 bg-slate-800/70 p-5">
                  <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-4 text-sm text-slate-300">
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-3">
                      {Object.entries(theme.colors).slice(0, 5).map(([label, color]) => (
                        <div key={label} className="rounded-3xl bg-slate-900 p-3 text-center">
